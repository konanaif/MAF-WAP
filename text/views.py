...
def index(request):
	...
	form = DataForm()
	return render(request, 'text/index.html', {'data_list':datas, 'form':form})
...
def upload(request):
	if request.method == 'POST':
		...
		return redirect(reverse('text:index'))
	
	else:
		form = ()
		files = glob('./' + request.path.split('/')[1] + '/data/*')

		return render(request, 'text/index.html', {'dl': files, 'form':form})
...
def load(request):
	if request.method == 'POST':
		...
		return render(request, 'text/data.html', context)
...
def metric(request, data_id):
	...
	return render(request, 'text/metric.html', context)
...
def mitigate(request):
	...
	return render(request, 'text/mitigate.html', context)