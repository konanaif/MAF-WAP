...
from .analyze import Kaifdata, KaifMitigation, Imagedata
...
def index(request):
	...
	form = DataForm()
	return render(request, 'image/index.html', {'data_list':datas, 'form':form})
...
def upload(request):
	if request.method == 'POST':
		...
		return render(request, 'image/index.html', {'dl': files, 'form':form})
...
def load(request):
	if request.method == 'POST':
		...
		data = Imagedata(data_index[0], datatype)  # Data loader
		...
		return render(request, 'image/data.html', context)
...
def mitigate(request):
	...
	return render(request, 'image/mitigate.html', context)