		...
		if n_analyzed == len(ids):
			...
			if datatype == 'text':
				...
				self.aif_data = aifData(df=vec_df,
					...
					protected_attribute_names=lconfig['protected_attribute_names'],
					privileged_classes=lconfig['privileged_classes'],
					features_to_drop=lconfig['features_to_drop']+['Rate'],
					na_values=lconfig['na_values'],
					categorical_features=lconfig['categorical_features']
				)
			else:
...
class Imagedata(Kaifdata):
	def __init__(self, id, datatype):
		...
		if data.aif_input_file_path:
			...
			with zipfile.ZipFile(data.datafile.name) as zf:
				zf.extractall(os.path.join('image', 'data'))

			# read label file
			label_df = pd.read_table(dconfig['labelFileName'], sep=dconfig['sep'], header=dconfig['header'])

			# image load
			images = glob.glob(os.path.join('data', dconfig['imageDirName'], '*[jpg$|png$|jpeg$|gif$]'))
...
class Audiodata(Kaifdata):
	def __init__(self, id, datatype):
		...
		if data.aif_input_file_path:
			...
			# audio load
			audios = glob.glob(os.path.join(dconfig['audioDirName'], '*[wav$|mp3$]'))
			...
			## make a dataframe
			df_obj = {dconfig['keyColname']: keys, 'Rate': rates}
			vectors = pad_sequences(np.array(vectors, dtype=object))
			vectors = vectors.transpose()

			for i, vect in enumerate(vectors):
				colname = 'Point_{0:020d}'.format(i)
				df_obj[colname] = vect

			audio_df = pd.DataFrame(df_obj)

			df = pd.merge(label_df, audio_df)
			df = df.drop_duplicates()
